...
import { loadDBProgressive, newId, earnRandom, skipRandom, recordSensible, requestWithdraw, setAuthToken, clearSavedCredentials, appendAnswer, adminUpdateUser, authMe, loadSettings, getAnsweredQuestionsInQuestionnaire, getUserQuestionnairesProgress, markQuestionnaireCompleted, syncQuestionnaireAnswers, resizeImage, clearDBCache, verifyPaymentStatus } from "../data/storage";
import {
...
export default function MainPage({ authUser, authPending }) {
  ...
  useEffect(() => {
    ...
    verifyPaymentStatus(userId).then((result) => {
      if (cancelled) return;
      if (result && result.ok) {
        if (result.fixed) {
          // Status was stuck, now fixed
          setUser((prev) => ({ ...prev, retrait: result.user.retrait, gagneSurBNI: result.user.gagneSurBNI }));
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (me && me.ok && me.user.retrait?.status !== "PENDING") {
          setUser((prev) => ({ ...prev, retrait: me.user.retrait, gagneSurBNI: me.user.gagneSurBNI }));
      ...
      try {
        ...
        if (result.ok) {
          ...
          for (const [qnId, progress] of Object.entries(result.progress)) {
            if (!progress?.completionRecorded && progress?.totalQuestions > 0 &&
  ...
  const questionnaireQuestions = useMemo(() => {
    ...
    if (missingQuestions && missingQuestions.length > 0) {
      ...
      return Array.from(uniq.values()).filter((q) => q && !answeredQuestionIds.has(String(q.id)));
    }
    ...
    return allQuestions.filter((q) => q && !answeredQuestionIds.has(String(q.id)));
  }, [db, currentQuestionnaire, missingQuestions, answeredQuestionIds]);

  const currentQuestion = mode === "QUESTIONNAIRE" ? (questionnaireQuestions[qnIndex] || null) : null;

  useEffect(() => {
    if (mode !== "QUESTIONNAIRE") return;
    if (currentQuestion) {
      console.log(`📍 Affichage de la question ${qnIndex}: ${currentQuestion.title} (ID: ${currentQuestion.id})`);
    } else {
  ...
  const finalizeQuestionnaireFlow = async (qnId) => {
    ...
    showQnOverlay("VALIDATING", { sub: "Merci de patienter…", autoHide: false });
      ...
      try {
        if (localAnswers.length > 0) {
          ...
          if (syncResult && syncResult.ok) {
            ...
            console.warn(`sync-answers attempt ${attempt} returned not-ok:`, syncResult);
          }
        } else {
          syncOk = true; // nothing to sync
        }
      } catch (e) {
        console.error(`Erreur sync finale questionnaire (tentative ${attempt}/3):`, e);
        if (attempt < 3) {
    ...
    if (!syncOk) {
      console.error("sync-answers: le rattrapage final a échoué. Les réponses déjà envoyées individuellement restent conservées.");
    }
    ...
    try {
      ...
      finalizeResult = { ok: false, error: "Finalisation impossible pour le moment." };
    }

    if (!finalizeResult || !finalizeResult.ok) {
      setQnFinalizeError(finalizeResult?.error || finalizeResult?.message || "La finalisation du questionnaire a échoué.");
      hideQnOverlay();
      setValidatingQuestionnaire(false);
      setSavingAnswer(false);
      return;
  ...
  const recordAnswerAndAdvance = async ({ questionId, answer, correct }) => {
    ...
    if (mode === "QUESTIONNAIRE") {
      ...
      setMissingQuestions((prev) =>
        Array.isArray(prev) ? prev.filter((qq) => String(qq?.id || "") !== String(questionId)) : []
      );
      setQnIndex(0);
      ...
      const remaining = allQnQuestions.filter((qq) => qq && !localAnswered.has(String(qq.id)));
              ...
              mode === "RANDOM" && !currentRandomQuestion ? (
                ...
                qnFinalizeError && !validatingQuestionnaire ? (
                  ...
                  <div className="errorCard loadingCard" aria-live="polite">
                    ...
                    <div className="errorMessage">On sauvegarde vos réponses et on crédite votre cagnotte.</div>
                    <div className="errorHint">Merci de patienter un instant.</div>
                  </div>
                )
              ) : (
                <>
                  <QuestionCard
                    key={mode === "RANDOM" ? (currentRandomQuestion?.id || "random-empty") : (currentQuestion?.id || "qn-empty")}
            ...
            {qnOverlayKind && mode === "QUESTIONNAIRE" ? (
              ...
              >
                ...
                <div className="skipOverlayInner">
                  ...
                  <div className="skipOverlayText">{qnOverlayKind === "VALIDATING" ? "Finalisation en cours" : "Réponse validée"}</div>
