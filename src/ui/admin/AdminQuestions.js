...
export default function AdminQuestions({ db, onDBChange }) {
  ...
  const qnById = useMemo(
    () => new Map((db.questionnaires || []).map((q) => [q.id, q])),
    [db],
  );
  ...
  const questions = useMemo(() => {
    ...
    if (filter === "QUESTIONNAIRE")
      base = base.filter((x) => !!x.questionnaire);

    // filter by captcha / prioritaire
    if (filter === "CAPTCHA")
      base = base.filter(
        (x) => String(x?.importance || "SENSIBLE").toUpperCase() === "CAPTCHA",
      );
    if (filter === "PRIORITY")
      base = base.filter((x) => !x.questionnaire && isPriorityActive(x));
  ...
  const editing = useMemo(
    () =>
      Array.isArray(db.questions)
        ? db.questions.find((q) => q.id === editingId)
        : null,
    [db, editingId],
  );
  ...
  const previewQ = useMemo(
    () =>
      Array.isArray(db.questions)
        ? db.questions.find((q) => q.id === previewId)
        : null,
    [db, previewId],
  );
    ...
    onDBChange((draft) => {
      ...
      if (payload.id) {
        ...
        if (idx >= 0) {
          ...
          const questionnaire =
            payload.questionnaire ?? existing.questionnaire ?? null;
          draft.questions[idx] = {
            ...existing,
            ...payload,
            questionnaire,
  ...
  const toggleActive = (q, desired) => {
    const qn = q.questionnaire
      ? getQuestionnaireById(db, q.questionnaire)
      : null;
    ...
    const lockedByActiveQuestionnaire =
      q.questionnaire && qn && isQuestionnaireActive(qn);
    ...
    <div>
      <div className="adminHeaderRow">
        <div>
          ...
          <div className="adminSub">
            L’endroit où on retrouve toutes les questions (individuelles +
            questionnaires).
          </div>
        </div>
      ...
      <div className="cardList" style={{ marginTop: 14 }}>
        {questions.map((q) => {
          ...
          const toggleDisabled = Boolean(
            q.questionnaire && qn && (qnActive || qnUnreleased),
          );
            ...
            <div key={q.id} className="adminCard glass">
              <div className="adminCardTop">
                ...
                <div className="adminCardTopRight">
                  <div className="adminRightPills">
                    ...
                    {tagName ? (
                      <span className="pill tagPill">{tagName}</span>
                    ) : null}
                    {q.questionnaire && qn ? (
                      <span className="pill qnPill">
                        Questionnaire: {qn.name}
                      </span>
                    ) : (
                    ...
                    {isPriorityActive(q) && q.priorityUntil ? (
                      <span className="pill priorityPill">
                        Prioritaire jusqu'au :{" "}
                        {formatPriorityUntil(q.priorityUntil)}
                      </span>
                    ) : null}
                  </div>
                  {/* Preview de l'image */}
              ...
              {["QCM", "DROPDOWN", "CHECKBOX"].includes(
                ...
                q.choices.length > 0 && (
                  <div className="questionChoicesPreview">
                    <div className="questionChoicesTitle">
                      Choix de réponse :
                    </div>
                    <div
                      className="questionChoicesList"
                    ...
                    >
                      {q.choices.map((choice, idx) => (
                        <div key={choice.id || idx} className="questionChoice">
                          ...
                          <span
                            className="questionChoiceText"
                            title={choice.text || `Choix ${idx + 1}`}
                          >
                            {choice.text || `Choix ${idx + 1}`}
                          </span>
                          {choice.isCorrect && (
                            <span className="questionChoiceCorrect">✓</span>
                          )}
                        </div>
                      ))}
                    </div>
              ...
              <div className="adminCardBottom">
                <div className="adminIconRow">
                  <IconButton title="Modifier" onClick={() => openEdit(q.id)}>
                    <Pencil size={18} />
                  </IconButton>
                  <IconButton
                ...
                <div className="adminToggleWrap">
                  ...
                  />
                  {qnUnreleased ? (
                    <div className="muted autoHint">
                      lié à un questionnaire non publié
                    </div>
                  ) : toggleDisabled ? (
                    <div className="muted autoHint">
                      lié à un questionnaire actif
                    </div>
                  ) : null}
                </div>
              </div>
      ...
      {answersQuestionId && (
        <ResponsesModal
          ...
          answers={(db.answers || []).filter(
            (a) => a.questionId === answersQuestionId,
          )}
