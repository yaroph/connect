...
function getUserName(r, userMap) {
  const direct = String(
    r.userName || r.userFullName || r.user || r.name || "",
  ).trim();
...
export default function ResponsesModal({
    ...
    const run = async () => {
      try {
        ...
        for (const u of list.users || list || []) {
          ...
          if (!id) continue;
          const name = String(
            u.fullName || `${u.prenom || ""} ${u.nom || ""}`.trim(),
          ).trim();
          if (name) map[id] = name;
        }
  ...
  const isQuestionnaire = useMemo(
    () => rows.some((r) => Boolean(r.questionnaireId)),
    [rows],
  );
  ...
  const isVariableUserTag = useMemo(() => {
    ...
    return rows.some(
      (r) => !r.questionId && r.userId && r.answer !== undefined,
    );
  }, [rows]);
  ...
  const canShowChart =
    chartData && chartData.some((item) => item.data !== null);
              ...
              return (
                <div key={item.questionId} className="chartCardResponse">
                  ...
                  <div className="chartNoData">
                    Pas de diagramme pour les photos
                  </div>
                </div>
              );
            }
            ...
            Array.from(byUser.entries()).map(([name, list]) => (
              <div key={name} className="respCard glassCard">
                <div className="respTop">
                  ...
                  <div className="pill">
                    {fmtDate(list[list.length - 1]?.createdAt)}
                  </div>
                </div>
                <div className="respAnswer" style={{ paddingTop: 8 }}>
                  {list.map((a) => (
                    <div
                      ...
                      onMouseEnter={(e) => {
                        const btn =
                          e.currentTarget.querySelector(".deleteAnswerBtn");
