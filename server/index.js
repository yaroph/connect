...
function normalizeUser(u) {
  ...
  const norm = {
    ...
    // lightweight activity / business tracking
    lastLoginAt: u.lastLoginAt || null,
    loginByDay: u.loginByDay && typeof u.loginByDay === "object" ? u.loginByDay : {},
    withdrawalsByMonth: u.withdrawalsByMonth && typeof u.withdrawalsByMonth === "object" ? u.withdrawalsByMonth : {},
...
function markUserLogin(user, value = new Date()) {
  ...
  const key = dayKey(at);
  const loginByDay = user && user.loginByDay && typeof user.loginByDay === "object"
    ? { ...user.loginByDay }
    : {};
...
async function mutateResponses(mutator, { attempts = 8 } = {}) {
  ...
  if (USE_BLOBS && !BLOBS_DISABLED) {
    try {
      const store = await getBlobsStore();
      if (!store) throw new Error("Netlify Blobs store not initialized");
      const key = String(RESPONSES_PATH);
...
app.get("/api/db", async (req, res) => {
  try {
    ...
    // Le mode admin (`scope=full`) ne doit pas être servi depuis un cache mémoire
    // local à une instance serverless, sinon une autre instance peut renvoyer un
    // snapshot obsolète juste après l'enregistrement d'une réponse.
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const user = markUserLogin(normalizeUser({
      prenom: b.prenom,
      nom: b.nom,
    ...
    const result = await runStateExclusive(async () => {
      ...
      return { token, user: next.find((u) => u.id === user.id) };
    });
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const result = await runStateExclusive(async () => {
      ...
      invalidateCache('users');
      return { token: users[idx].token, user: users[idx] };
    });
...
app.put("/api/user/me", async (req, res) => {
  try {
    ...
    const result = await runStateExclusive(async () => {
      ...
      if (idx === -1) return { error: "Invalid token", status: 401 };
  ...
  try {
    ...
    const result = await responsesMutex.runExclusive(async () => (
      runStateExclusive(async () => {
        ...
        if (cagnotte[id]) delete cagnotte[id];
        ...
        await Promise.all([
          writeUsers(users),
          mutateResponses(async (responses) => {
            responses.answers = (responses.answers || []).filter((a) => a.userId !== id);
            responses.completions = (responses.completions || []).filter((c) => c.userId !== id);
            return { ok: true };
          }),
...
app.post("/api/earn/questionnaire", async (req, res) => {
  try {
    const { userId, amount, questionnaireId } = req.body || {};
...
// Finaliser et créditer un questionnaire sans recontrôle bloquant des questions manquantes.
...
// Endpoint idempotent de complétion, utilisé aussi comme rattrapage automatique.
...
app.post("/api/user/request-withdraw", async (req, res) => {
  try {
    ...
    const result = await runStateExclusive(async () => {
      ...
      u.withdrawalsByMonth = u.withdrawalsByMonth && typeof u.withdrawalsByMonth === "object" ? u.withdrawalsByMonth : {};
...
// Verify if a user's PENDING payment actually exists in the admin payment list.
// If not, the status is stuck and should be reset to IDLE.
app.post("/api/user/verify-payment-status", async (req, res) => {
  try {
    const { userId } = req.body || {};
    if (!userId) return res.status(400).json({ error: "userId requis" });
    const result = await runStateExclusive(async () => {
...
app.post("/api/admin/payments/:id/validate", async (req, res) => {
  try {
    ...
    const result = await runStateExclusive(async () => {
      ...
      invalidateCache('users');
      return { ok: true, user: users[idx], remaining: next.length };
    });
...
app.post("/api/admin/payments/:id/cancel", async (req, res) => {
  try {
    ...
    const result = await runStateExclusive(async () => {
      ...
      invalidateCache('users');
      invalidateCache('cagnotte');
      return { ok: true, user: users[idx], pending: cagnotte[p.userId].pending, remaining: next.length };
    });
...
app.get("/api/admin/statistics", async (req, res) => {
  try {
    ...
    // Connexions par jour (basé sur les logins réels persistés sur le profil utilisateur)
    users.forEach((user) => {
      const loginByDay = user && user.loginByDay && typeof user.loginByDay === "object"
        ? user.loginByDay
        : {};
